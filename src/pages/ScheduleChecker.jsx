...
// ── Deduplicate helpers (fixes repeated room/date chips from n8n) ─────────────
function dedupeRooms(rooms) {
  const seen = new Set();
  return (rooms || []).filter(r => {
    const key = r.room_id || r.Room_ID || r.room_name || r.Room_Name || JSON.stringify(r);
...
export default function ScheduleChecker({ onBack }) {
  const [form, setForm] = useState({
    ...
    start_time: "7:30 AM", end_time: "12:30 PM", room_id: "", notes: "",
  });
  ...
  // ── Click room chip → fills Room ID field + shows hint ──────────────────
  const handleRoomClick = (room) => {
    ...
    setAlert(prev => ({
      ...prev,
      hint: `✅ Room ID set to "${roomId}" (${roomName}). Click Check Schedule to recheck.`,
    }));
  };

  // ── Click date chip → fills Date field + shows hint ─────────────────────
  const handleDateClick = (dateItem) => {
    const d = dateItem.date || dateItem;
    setSelectedDate(d);
    setSelectedRoom(null);
    update("date", d);
    setAlert(prev => ({
      ...prev,
      hint: `✅ Date set to ${formatDate(d)}. Click Check Schedule to recheck.`,
    }));
  };
  ...
  const iStyle = (field) => ({
    ...
    border: `1.5px solid ${focusField === field ? "#10b981" : "#1e3a5a"}`,
    borderRadius: "10px", padding: "11px 14px",
    color: "#e2e8f0", fontSize: "14px", outline: "none",
    fontFamily: "'Plus Jakarta Sans', sans-serif", transition: "all 0.18s",
    boxShadow: focusField === field ? "0 0 0 3px rgba(16,185,129,0.12)" : "none",
  });
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      if (data.success) {
        setAlert({ type: "success", title: "Schedule is Clear ✅", aiMessage: data.message, reasoning: data.data?.ai_reasoning || null, hint: null });
        setHistory(h => [{ date: form.date, employee: form.employee_id, shift: form.shift_type, room: form.room_id || "—", status: "Clear" }, ...h]);
      } else {
        const conflicts = (data.data?.conflicts || []).map(c =>
          typeof c === "string" ? { rule: c, severity: "Critical", detail: c } : c
        );
        // ── Deduplicate here before storing in state ──
        const altRooms = dedupeRooms(data.data?.alternatives?.rooms || []);
        const altDates = dedupeDates(data.data?.alternatives?.dates || []);

        setAlert({
          type: "error", title: "Schedule Conflict Found ⚠️",
          aiMessage: data.message,
          reasoning: data.data?.ai_reasoning || null,
          conflicts, altRooms, altDates,
          suggestedDate: data.data?.suggested_date || null,
          suggestedRoom: data.data?.suggested_room || null,
          hint: null,
        });
        setHistory(h => [{ date: form.date, employee: form.employee_id, shift: form.shift_type, room: form.room_id || "—", status: "Conflict" }, ...h]);
      }
    } catch (e) {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hist-row:hover { background: rgba(16,185,129,0.04) !important; }
        ...
        .back-btn:hover { background:rgba(16,185,129,0.12) !important; color:#6ee7b7 !important; }
        .shift-chip:hover { border-color:#10b981 !important; color:#34d399 !important; }

        /* ── Clickable alternative chips ───────────────────────────── */
        .alt-date-chip {
        ...
        .alt-date-chip:active { transform: translateY(0px); }
        .alt-date-chip.selected {
          background: rgba(34,197,94,0.22) !important;
          border-color: #22c55e !important;
          box-shadow: 0 0 0 3px rgba(34,197,94,0.18), 0 4px 16px rgba(34,197,94,0.2);
          color: #86efac;
        }
        ...
        .alt-room-chip:active { transform: translateY(0px); }
        .alt-room-chip.selected {
          background: rgba(99,102,241,0.22) !important;
          border-color: #818cf8 !important;
          box-shadow: 0 0 0 3px rgba(99,102,241,0.18), 0 4px 16px rgba(99,102,241,0.2);
          color: #c7d2fe;
        }
      `}</style>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              {onBack && (
                <button className="back-btn" onClick={onBack}
                  style={{ background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: "10px", padding: "8px 14px", color: "#34d399", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: "'Plus Jakarta Sans', sans-serif", transition: "all 0.18s", display: "flex", alignItems: "center", gap: "6px" }}>
                  ← Back
                </button>
              )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.15fr 0.85fr", gap: "20px", alignItems: "start" }}>
            ...
            <div style={{ background: "#0c1525", border: "1px solid #1e3a5a", borderRadius: "20px", overflow: "hidden" }}>
              ...
              <div style={{ padding: "24px" }}>
                ...
                <div style={{ marginBottom: "14px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    {Object.keys(SHIFT_TIMES).map(shift => (
                      <button key={shift} className="shift-chip" onClick={() => handleShiftChange(shift)}
                        style={{ padding: "8px 18px", borderRadius: "8px", border: `1.5px solid ${form.shift_type === shift ? "#10b981" : "#1e3a5a"}`, background: form.shift_type === shift ? "rgba(16,185,129,0.12)" : "#0c1525", color: form.shift_type === shift ? "#34d399" : "#475569", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: "'Plus Jakarta Sans', sans-serif", transition: "all 0.15s" }}>
                ...
                {/* ALERT */}
                {alert && (
                  <div style={{ marginTop: "16px", borderRadius: "14px", overflow: "hidden", animation: "slideDown 0.25s ease", border: `1px solid ${alert.type === "success" ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.3)"}` }}>

                    <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: "9px", background: alert.type === "success" ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)" }}>
                      <span style={{ fontSize: "18px" }}>{alert.type === "success" ? "✅" : "❌"}</span>
                      <span style={{ fontWeight: "700", fontSize: "14px", color: alert.type === "success" ? "#4ade80" : "#f87171" }}>{alert.title}</span>
                    </div>
                    ...
                    <div style={{ padding: "14px 16px", background: "rgba(0,0,0,0.3)" }}>
                      ...
                      {alert.aiMessage && (
                        <div style={{ display: "flex", gap: "10px", alignItems: "flex-start", marginBottom: (alert.conflicts?.length || alert.altRooms?.length || alert.altDates?.length) ? "14px" : 0 }}>
                          ...
                          <div style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.15)", borderRadius: "4px 12px 12px 12px", padding: "10px 14px", flex: 1 }}>
                            ...
                            {alert.reasoning && <p style={{ fontSize: "11px", color: "#475569", marginTop: "6px", fontStyle: "italic" }}>Reason: {alert.reasoning}</p>}
                          </div>
                        </div>
                      )}

                      {/* 💡 Hint shown after clicking a chip */}
                      {alert.hint && (
                        <div style={{ marginBottom: "12px", padding: "9px 14px", background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: "8px", display: "flex", alignItems: "center", gap: "8px", animation: "slideDown 0.2s ease" }}>
                          <span style={{ fontSize: "13px" }}>💡</span>
                          <span style={{ fontSize: "12px", color: "#34d399", fontWeight: "600" }}>{alert.hint}</span>
                        </div>
                      )}
                      ...
                      {/* ── Alternative Rooms — clickable, deduplicated ── */}
                      {alert.altRooms && alert.altRooms.length > 0 && (
                        <div style={{ marginTop: "14px" }}>
                          <div style={{ fontSize: "10px", color: "#475569", textTransform: "uppercase", letterSpacing: "0.6px", fontWeight: "700", marginBottom: "8px" }}>
                            🏠 Available Alternative Rooms
                            <span style={{ marginLeft: "6px", color: "#1e3a5a", fontWeight: "500", textTransform: "none", letterSpacing: 0 }}>— click to use</span>
                          </div>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {alert.altRooms.map((r, i) => {
                              const roomId = r.room_id || r.Room_ID;
                              const isSelected = selectedRoom === roomId;
                              return (
                                <button key={i}
                                  className={`alt-room-chip${isSelected ? " selected" : ""}`}
                                  onClick={() => handleRoomClick(r)}>
                                  {isSelected ? "✓" : "🏠"} {r.room_name || r.Room_Name}
                      ...
                      {/* ── Alternative Dates — clickable, deduplicated ── */}
                        ...
                        <div style={{ marginTop: "14px" }}>
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {alert.altDates.map((a, i) => {
                              const dateVal = a.date || a;
                              const isSelected = selectedDate === dateVal;
                              return (
                                <button key={i}
                                  className={`alt-date-chip${isSelected ? " selected" : ""}`}
                                  onClick={() => handleDateClick(a)}>
                                  {isSelected ? "✓" : "📅"} {formatDate(dateVal)}{a.weekday ? ` · ${a.weekday}` : ""}
                                </button>
                              );
                            })}
                          </div>
                ...
                <div style={{ padding: "4px 0", maxHeight: "340px", overflowY: "auto" }}>
                  {history.length === 0 ? (
                    ...
                    <div key={i} className="hist-row" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 22px", borderBottom: i < history.length - 1 ? "1px solid #0f1e30" : "none", transition: "background 0.15s", gap: "10px" }}>
                      ...
                      <span style={{ fontSize: "11px", fontWeight: "700", color: h.status === "Clear" ? "#22c55e" : "#ef4444", background: h.status === "Clear" ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.08)", border: `1px solid ${h.status === "Clear" ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"}`, borderRadius: "6px", padding: "3px 10px", display: "flex", alignItems: "center", gap: "5px", whiteSpace: "nowrap", flexShrink: 0 }}>
